...
const getAllTodos = async (req, res) => {
  try {
    ...
    const { userId, priority, tags, sortBy , sortOrder } = req.query;
    ...
    // Build filter object
    const filter = {
      $or: [{ createdBy: req.user._id }, { mentions: req.user._id }],
    };
...
const getTodo = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new AppError("You do not have permission to access this todo", 403)
    );
  }
...
const updateTodo = catchAsync(async (req, res, next) => {
  ...
  if (!todo.createdBy.equals(req.user._id)) {
    return next(
      new AppError("You do not have permission to update this todo", 403)
    );
  }
...
const deleteTodo = catchAsync(async (req, res, next) => {
  ...
  if (!todo.createdBy.equals(req.user._id)) {
    return next(
      new AppError("You do not have permission to delete this todo", 403)
    );
  }
...
const addNote = catchAsync(async (req, res, next) => {
  ...
  // Check if user has access to this todo
  if (
    !todo.createdBy.equals(req.user._id) &&
    !todo.mentions.some((userId) => userId.equals(req.user._id))
  ) {
    return next(
      new AppError("You do not have permission to add notes to this todo", 403)
    );
  }
...
export { getAllTodos, getTodo, createTodo, updateTodo, deleteTodo, addNote };
